...
const isROwner = [botJid, ...global.owner.map(([v]) => normalizeJid(v))].includes(senderJid)
const isOwner = isROwner || m.fromMe
const isMods = isOwner || global.mods.map(v => normalizeJid(v)).includes(senderJid)
const isPrems = isROwner || global.prems.map(v => normalizeJid(v)).includes(senderJid) || _user?.prem == true
