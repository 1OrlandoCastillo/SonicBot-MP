...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    for (let name in global.plugins) {
      ...
      const noPrefix = m.text.slice(prefixMatch.index + prefixMatch[0].length).trim()

      let [command, ...args] = noPrefix.split(/\s+/g)
      command = (command || '').toLowerCase() // ✅ FIX aplicado aquí
