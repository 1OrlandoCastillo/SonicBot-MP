...
const defaultMenu = {
  before: `
• ${namebot}
*﹙ ✿ ﹚Channel Official* [https://whatsapp.com/channel/0029VbAZUQ3002T9KZfx2O1M](https://whatsapp.com/channel/0029VbAZUQ3002T9KZfx2O1M)
*﹙ ✿ ﹚Akirax Host* [https://home.akirax.net](https://home.akirax.net)
%readmore`.trimStart(),
...
const handler = async (m, { conn, usedPrefix: _p, __dirname }) => {
  try {
    ...
    const dateIslamic = new Intl.DateTimeFormat(`${locale}-TN-u-ca-islamic`, { day: 'numeric', month: 'long', year: 'numeric' }).format(d)
    ...
    let text = _text.replace(new RegExp(`%(${Object.keys(replace).sort((a, b) => b.length - a.length).join('|')})`, 'g'), (_, name) => String(replace[name]))

    const jid = m.sender.split('@')[0]
    const path1 = `./storage/menu/${jid}_1.jpg`
    const path2 = `./storage/menu/${jid}_2.jpg`

    const exists1 = await fs.stat(path1).then(() => true).catch(() => false)
    const exists2 = await fs.stat(path2).then(() => true).catch(() => false)

    const image1 = exists1 ? path1 : './storage/img/default1.jpg'
    const image2 = exists2 ? path2 : './storage/img/default2.jpg'

    await conn.sendMessage(m.chat, { image: { url: image1 }, caption: text }, { quoted: m })
    await conn.sendMessage(m.chat, { image: { url: image2 }, caption: '• Anya Forger' }, { quoted: m })
