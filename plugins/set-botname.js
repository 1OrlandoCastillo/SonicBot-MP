...
const handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) return m.reply(`👀 Usa así: *${usedPrefix + command} nombre nuevo*`, m, rcanal)
  ...
  if (!fs.existsSync(botPath)) {
    return m.reply('❌ No encontré tu sub bot activo.', m, rcanal)
  }
  ...
  try {
    ...
    m.reply(`☁︎ Nombre del sub bot cambiado a: *${text.trim()}*`, m, rcanal)
  } catch (err) {
    console.error(err)
